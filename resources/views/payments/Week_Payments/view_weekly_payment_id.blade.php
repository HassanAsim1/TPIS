  ...
  @if(session()->has('success'))
    ...
    <form action="{{url('weekly_payments')}}" method="POST" id="weeksubmit">
      @csrf
		...
		<hr class="mt-4">
        @php $count = 0; $totalEmployees = count($weekpay) @endphp
            @foreach($weekpay as $index => $employee)
                            <div class="row" id="addnewrow" style="margin-top:10px;">
                                <div class="col">
                                  <input type="text" class="form-control mt-2" name="employee_id[]" value="{{$employee->emp_id}}" placeholder="EMP ID" readonly/>
                                </div>
                                <div class="col">
                                  <input type="text" class="form-control mt-2" value="{{$employee->name}}" name="name[]" placeholder="EMP NAME" readonly/>
                                </div>
                                <div class="col">
                                <input type="text" class="form-control mt-2 tquan" id="squan{{$count}}" value="{{$employee->amount}}" name="debit[]" placeholder="debit" readonly/>
                                </div>
                                <div class="col">
                                  <input type="text" class="form-control mt-2 signature-input" name="signature[]" value="{{$employee->signature}}" placeholder="Signature" readonly/>
                                </div>
                                {{-- <div class="col">
                            ...
                            <div class="row justify-content-end">
                              ...
                              <input type="text" class="form-control" id="total_amount" name="total_amount" value="{{$data->total_amount}}" placeholder="Total Quantity" readonly/>
                              </div>
                              <div class="col-sm-2 mb-3">
                                <label class="col-form-label" for="basic-default-company">Recieved Amount:</label>
                                <input type="number" class="form-control stotal" id="recieve_amount" name="recieved_amount" value="{{$data->recieved_amount}}" placeholder="Recieve Amount" readonly/>
                              </div>

                              <div class="col-sm-2 mb-3">
                                <label class="col-form-label" for="basic-default-company">Remaining Amount:</label>
                                <input type="text" class="form-control" id="remain_amount" name="remaining_amount" value="{{$data->remaining_amount}}" placeholder="Recieve Amount" readonly />
                              </div>
                            </div>
                          </div>
                      </form>
